...
function extract_public_key {
    ...
    RESULT=$(echo $1 | sed 's/.*"public_key"://g' | sed 's/,.*//g' | sed 's/}//g')
...
USER_CREATION_OUTPUT=$(curl -u admin:pass -H "Content-Type: application/json" -X POST -d "{\"username\": \"$USER\", \"project\": \"$PROJECT\", \"password\": \"$PASSWORD\"}" $RESOURCE_MANAGER_URL/users/)
...
set +x
#PUBLIC_KEY_GET_OUTPUT=$(curl -H "TOKEN: $TOKEN" -X GET $RESOURCE_MANAGER_URL/rsa_public_key/$USER_ID/ | extract_public_key)
PUBLIC_KEY_GET_OUTPUT=$(curl -H "TOKEN: $TOKEN" -X GET $RESOURCE_MANAGER_URL/rsa_public_key/$USER_ID/ | sed 's/.*"public_key":"//g' | sed 's/,.*//g' | sed 's/"}//g')
echo -e "$PUBLIC_KEY_GET_OUTPUT" > public_key.txt
...
# echo "$OS_PASSWORD_INPUT" > encrypted.txt


cat > encrypted.txt <<- EOM
{
   "username": "jpastor2",
   "password": "plop2",
   "project": "FG-3922"
...
cat encrypted.txt | openssl rsautl -encrypt -pubin -inkey public_key.txt > cipher.txt
ENCRYPTED_VALUE=$(cat encrypted.txt | openssl rsautl -encrypt -pubin -inkey public_key.txt)
...
read -r -d '' CREDENTIALS_JSON_VALUE <<- EOM
{
...
echo "$CREDENTIALS_JSON_VALUE"

# Send the encrypted password to the webservice
curl -u admin:pass -H "TOKEN: $TOKEN"  -X POST --header 'Content-Type: application/json' --header 'Accept: application/json' -d "$CREDENTIALS_JSON_VALUE" $RESOURCE_MANAGER_URL/credentials/
rm -rf cipher.txt
